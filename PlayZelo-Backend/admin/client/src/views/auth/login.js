...
import { userLogin, confirmAuth } from "redux/action/auth";
...
const Login = () => {
            ...
            if(formData.password !== '') {
                ...
                if (response.status) {
                    if(response.otp) {
                        ...
                        Config.Api.setToken(response.customer.token);
    ...
    const handleTwoFactorAuth = async () => {
        ...
        await confirmAuth(requestData, addToast);
    }
        ...
        <Box className={classes.LoginContainer}>
            {
                pageState === 0 ? (
                    ...
                    <Box className={classes.FormBox}>
                        <Box className={classes.InputContainer}>
                            <p className={classes.CenterText}>Please Input your 2FA code</p>
                        </Box>
                        <Box className={classes.InputContainer}>
                            <input type='text' placeholder='2FA Code' value={tFACode} onChange={(e) => setTFACode(e.target.value)} className={classes.FormInput} />
                        </Box>
                        <Grid container className={classes.ActionBox} justifyContent="flex-end">
                            <Grid item xs={4}>
                                <Grid container justifyContent="flex-end">
                                    <Button variant="contained" onClick={handleTwoFactorAuth}>Send Code</Button>
                                </Grid>
                            </Grid>
                        </Grid>
                    </Box>
