...
import { loadDashBoardData, getMinesData } from "redux/action/report";
...
const headCells = [
    ...
    { value: 'minesCount', label: 'Mines Count', ischeck: true },
...
const MinesManagement = () => {
    ...
    const init = async () => {
        ...
        if (response.status) {
            setTotalRows(Number(response.minesCount));
            const requestData = {
                id: userId,
                offset: page,
                count: rowsPerPage
            }
            const response1 = await getMinesData(requestData);
            if (response1.status) {
                setData(response1.minesData);
            }
        }
        hideLoading();
    };
                    ...
                    <TableBody>
                        {
                            data?.map((item) => (
                                <TableRow key={item._id} className={classes.TableRow}>
                                    ...
                                    <TableCell>
                                        {(item.roundResult === 'payout' || item.roundResult === 'finish') ? item.payout : '---'}
                                    </TableCell>
                                </TableRow>
                            ))
                        }
...
export default MinesManagement;