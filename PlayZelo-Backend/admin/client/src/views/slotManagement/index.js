...
import { loadDashBoardData, getSlotData } from "redux/action/report";
...
const SlotManagement = () => {
    ...
    const init = async () => {
        ...
        if (response.status) {
            setTotalRows(Number(response.slotCount));
            const requestData = {
                id: userId,
                offset: page,
                count: rowsPerPage
            }
            const response1 = await getSlotData(requestData);
            if (response1.status) {
                setData(response1.slotData);
            }
        }
        hideLoading();
    };
