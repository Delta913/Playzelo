...
import { loadDashBoardData, getScissorsData } from "redux/action/report";
...
const headCells = [
    ...
    { value: 'betNumber', label: 'Player', ischeck: true },
    { value: 'winNumber', label: 'Dealar', ischeck: true },
...
const ScissorsManagement = () => {
    ...
    const init = async () => {
        ...
        if (response.status) {
            setTotalRows(Number(response.scissorsCount));
            const requestData = {
                id: userId,
                offset: page,
                count: rowsPerPage
            }
            const response1 = await getScissorsData(requestData);
            if (response1.status) {
                setData(response1.scissorsData);
            }
        }
        hideLoading();
    };
                    ...
                    <TableBody>
                        {
                            data?.map((item) => (
                                <TableRow key={item._id} className={classes.TableRow}>
                                    ...
                                    <TableCell>
                                        {item.betNumber === 0 ? 'Rock' : item.betNumber === 1 ? 'Scissor' : 'Paper'}
                                    </TableCell>
                                    <TableCell>
                                        {item.winNumber === 0 ? 'Rock' : item.winNumber === 1 ? 'Scissor' : 'Paper'}
                                    </TableCell>
...
export default ScissorsManagement;