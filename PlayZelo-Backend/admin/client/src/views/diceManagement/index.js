...
import { loadDashBoardData, getDiceData } from "redux/action/report";
...
const DiceManagement = () => {
    ...
    const init = async () => {
        ...
        if (response.status) {
            setTotalRows(Number(response.diceCount));
            const requestData = {
                id: userId,
                offset: page,
                count: rowsPerPage
            }
            const response1 = await getDiceData(requestData);
            if (response1.status) {
                    ...
                    <TableBody>
                        {
                            data?.map((item) => (
                                <TableRow key={item._id} className={classes.TableRow}>
                                    ...
                                    <TableCell>
                                        {(item.roundResult === 'win') ? item.payout : '---'}
                                    </TableCell>
                                </TableRow>
                            ))
                        }
